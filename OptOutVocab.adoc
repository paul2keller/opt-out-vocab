...
This list of categories may be expanded with categories adressing additional uses cased should such use cases emerge. In addition to these categories definined in the vocabulary, it is also expected that some systems implementing this vocabulary may extend this list with additional categories for their particular needs. 

=== Relationship

The `TDM` category is the overarching category that includes not only its own use cases, but also the various types of AI training, as they are considered to be forms of TDM.  As such, if a rightsholder opts out of / reserves the rights to `TDM`, they are opting out of those other categories as well. The other categories can be independtly adressed and affect only the specifi use case covered by the category. 
